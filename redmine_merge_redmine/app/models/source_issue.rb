  ...
  def self.migrate
    all.each do |source_issue|
      issue = Issue.create!(source_issue.attributes) do |i|
        puts "ISSUE ID : #{source_issue.id}"
        i.project = Project.find(RedmineMerge::Mapper.get_new_project_id(source_issue.project.id))
        ...
        #if source_issue.fixed_version_id!=nil
	#    puts "VERSION NOT NULL"
	#    i.fixed_version = Version.find(RedmineMerge::Mapper.get_new_version_id(source_issue.fixed_version_id))      
    	#    puts "Dest : fixed-version_id : #{i.fixed_version.id} #{i.fixed_version.project_id}"
        #end
      end
